package fr.kohen.alexandre.framework.engine.resources;
...
public class ResourceManager {
	...
	public static void setString(String key, String file) {
		strings.put(key, file);
	}
	...
	private static void loadFile(Element stringval) throws SlickException {
		String key = stringval.getAttribute("key");
		String value = stringval.getAttribute("file");
		setFile(key, value);
	}

	public static void setFile(String key, String value) {
		files.put(key, baseDir + value);
	}

	public static String getFile(String key) {
		String val = files.get(key);
		if (val == null)
			Log.error("No file for key" + key + " found!");
		return val;
	}
